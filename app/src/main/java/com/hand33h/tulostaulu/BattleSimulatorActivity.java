...
public class BattleSimulatorActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        root.addView(info("Hero-puoli käyttää nyt automaattisesti varmennettuja Expedition-bonuksia sekä Legendary-herojen Exclusive Weapon Expedition Lethality/Health -arvoja Gen 1–17. Ehdollisia 40 % proc-, kierros- ja kohdekohtaisia skillejä ei muuteta keksityksi keskiarvoksi. Hero Gear -tasot tallennetaan kuormaan, mutta niiden Command-bonus jätetään manuaaliseen verified-kenttään kunnes laatu/forging/mastery on mukana."));
    ...
    private void buildSide(boolean attacker){
        ...
        for(int i=0;i<3;i++){
            ...
            root.addView(tiny("Expedition skills level (1–5, same selected level for the 3 skills)"));sk[i]=spinner(HeroBattleData.SKILL_LEVELS);sk[i].setSelection(4);root.addView(sk[i]);
            root.addView(tiny("Exclusive / Special Weapon level"));ew[i]=spinner(HeroBattleData.EXCLUSIVE_LEVELS);root.addView(ew[i]);
            for(int g=0;g<4;g++){root.addView(tiny(HeroBattleData.HERO_GEAR_SLOTS[g]+" level (tracked)"));hg[i][g]=spinner(HeroBattleData.HERO_GEAR_LEVELS);root.addView(hg[i][g]);}
        }
    ...
    private void simulate(){
        try{
            ...
            result.setText(String.format(Locale.US,"🏆 Predicted winner: %s\n\nWin estimate\nAttacker %.1f %% | Defender %.1f %%\n\nEffective weighted stats\nA: ATK %.2f DEF %.2f HP %.2f LETH %.2f DMG +%.2f%%\nD: ATK %.2f DEF %.2f HP %.2f LETH %.2f DMG +%.2f%%\n\nEffective squad size\nA: %,.0f | D: %,.0f\n\nCombat score\nA: %,.0f | D: %,.0f\n\nEstimated losses\nA: %.1f %% (~%,d left)\nD: %.1f %% (~%,d left)\n\nPet\nA: %s\nD: %s\n\nAUTO HERO DATA — ATTACKER\n%s\nAUTO HERO DATA — DEFENDER\n%s\n\nHero EW Expedition Lethality/Health is applied automatically by hero troop type. Conditional proc/turn skills are shown in notes but are not converted into invented averages. Manual Hero fields are additive.",p>=.5?"ATTACKER":"DEFENDER",p*100,(1-p)*100,A.s.atk,A.s.def,A.s.hp,A.s.leth,A.s.damageDealt,D.s.atk,D.s.def,D.s.hp,D.s.leth,D.s.damageDealt,A.count,D.count,as,ds,aLoss,ar,dLoss,dr,apr.note,dpr.note,aHeroNotes,dHeroNotes));
    ...
    private String applyHeroes(Side own,Side enemy,Spinner[] heroes,Spinner[] skills,Spinner[] exclusive,boolean attackerSide){
        ...
        for(int i=0;i<3;i++){
            String selected=(String)heroes[i].getSelectedItem();
            if(selected==null||"None".equals(HeroBattleData.nameOf(selected))) continue;
            int lv=Integer.parseInt((String)skills[i].getSelectedItem());
            int ewLv=Integer.parseInt((String)exclusive[i].getSelectedItem());
            ...
            own.s.atk+=weightedTroop(own,e.infAtk,e.lanAtk,e.marAtk);
            own.s.def+=weightedTroop(own,e.infDef,e.lanDef,e.marDef);
            own.s.hp+=weightedTroop(own,e.infHp,e.lanHp,e.marHp);
            own.s.leth+=weightedTroop(own,e.infLeth,e.lanLeth,e.marLeth);

            double ew=HeroBattleData.exclusiveExpeditionLethHp(selected,ewLv);
            String type=HeroBattleData.troopTypeOf(selected);
            double share="Infantry".equals(type)?own.inf:("Lancer".equals(type)?own.lan:("Marksman".equals(type)?own.mar:0));
            double weightedEw=ew*share/100.0;own.s.leth+=weightedEw;own.s.hp+=weightedEw;

            notes.append("• ").append(HeroBattleData.nameOf(selected)).append(" skill Lv").append(lv).append(": ");
            if(e.hasVerifiedSkillData) notes.append(e.note); else notes.append("skill semantics not yet auto-scored; use Additional verified Hero fields for direct bonuses");
            notes.append(" | ").append(HeroBattleData.exclusiveSummary(selected,ewLv)).append("\n");
        }
        if(notes.length()==0) return "No heroes selected";
    ...
    private void applyGearAndCharms(Side x,Spinner[] gear,Spinner[] charm){
        double infGear=(GEAR_BONUS[gear[2].getSelectedItemPosition()]+GEAR_BONUS[gear[3].getSelectedItemPosition()])/2.0;
        double lanGear=(GEAR_BONUS[gear[0].getSelectedItemPosition()]+GEAR_BONUS[gear[1].getSelectedItemPosition()])/2.0;
        double marGear=(GEAR_BONUS[gear[4].getSelectedItemPosition()]+GEAR_BONUS[gear[5].getSelectedItemPosition()])/2.0;
        double weighted=(x.inf*infGear+x.lan*lanGear+x.mar*marGear)/100.0;x.s.atk+=weighted;x.s.def+=weighted;
        int cap=0;for(Spinner g:gear)cap+=GEAR_CAP[g.getSelectedItemPosition()];x.count+=cap;
        double infC=0,lanC=0,marC=0;for(int i=0;i<18;i++){double b=CHARM_BONUS[charm[i].getSelectedItemPosition()];if(i<6)lanC+=b;else if(i<12)infC+=b;else marC+=b;}
        infC/=6.0;lanC/=6.0;marC/=6.0;double c=(x.inf*infC+x.lan*lanC+x.mar*marC)/100.0;x.s.hp+=c;x.s.leth+=c;
    }

    private void applyExpert(Side own,Side enemy,String name,int affinity,boolean skills){
        double k=affinity/100.0;
        if("Agnes".equals(name))own.s.def+=15*k;
        else if("Cyrille".equals(name))own.s.atk+=15*k;
        else if("Holger".equals(name)){own.s.atk+=15*k;own.s.def+=15*k;}
        else if("Romulus".equals(name)){own.s.leth+=20*k;own.s.hp+=20*k;if(skills){own.s.atk+=10;own.s.def+=10;own.s.leth+=10;own.s.hp+=10;own.count+=10000;}}
        else if("Baldur".equals(name)){own.s.atk+=10*k;own.s.def+=10*k;}
        else if("Fabian".equals(name)){own.s.leth+=15*k;own.s.hp+=15*k;}
        else if("Valeria".equals(name)){own.s.leth+=20*k;own.s.hp+=20*k;if(skills){own.s.atk+=30;own.s.def+=30;own.s.leth+=30;own.s.hp+=30;}}
        else if("Ronne".equals(name)){own.s.atk+=15*k;own.s.def+=15*k;if(skills){own.s.atk+=30;own.s.def+=30;}}
        else if("Kathy".equals(name)){own.s.leth+=15*k;own.s.hp+=15*k;}
        else if("Gareth".equals(name)){own.s.atk+=30*k;own.s.hp+=30*k;if(skills){own.s.def+=50;own.s.hp+=50;enemy.s.leth-=5;}}
    }

    private PetResult applyPet(Stats own,Stats enemy,String pet,int level){
        int i=Math.max(0,Math.min(9,level/10-1));double[] p={2.5,3,3.5,4,5,6,7,8,9,10};
        if("Cave Lion".equals(pet)){own.atk+=p[i];return new PetResult(0,"ATK +"+p[i]+"%");}
        if("Saber-tooth Tiger".equals(pet)){own.leth+=p[i];return new PetResult(0,"Lethality +"+p[i]+"%");}
        if("Mammoth".equals(pet)){own.def+=p[i];return new PetResult(0,"Defense +"+p[i]+"% (range pattern; verify client stage)");}
        if("Frost Gorilla".equals(pet)){own.hp+=p[i];return new PetResult(0,"Health +"+p[i]+"%");}
        if("Frostscale Chameleon".equals(pet)){enemy.def-=p[i];return new PetResult(0,"Enemy Defense -"+p[i]+"% (range pattern; verify client stage)");}
        if("Snow Leopard".equals(pet)){double red=(i>=9?5:new double[]{1.5,2,2.5,3,3.5,4,4,4,4,5}[i]);enemy.leth-=red;return new PetResult(0,"Enemy Lethality -"+red+"%; intermediate late stages not claimed exact");}
        if("Titan Roc".equals(pet)){double[] known={1.5,2,2.5,3,3.5,4,5};int k=Math.min(6,(int)Math.floor(i*6.0/9.0));double red=known[k];enemy.hp-=red;return new PetResult(0,"Enemy Health -"+red+"%; mapped to 7 published skill stages");}
        if("Snow Ape".equals(pet)){double c=1500.0*(i+1);return new PetResult(c,"Squad Capacity +"+(int)c);}
        if("Iron Rhino".equals(pet))return new PetResult(0,"Rally Capacity bonus; not added to single-march count");
        return new PetResult(0,"No direct combat stat modifier");
    }

    private double armyScore(Side x){
        double a=Math.max(-95,x.s.atk),d=Math.max(-95,x.s.def),h=Math.max(-95,x.s.hp),l=Math.max(-95,x.s.leth),dmg=Math.max(-95,x.s.damageDealt);
        return x.count*Math.sqrt((1+a/100)*(1+l/100)*(1+d/100)*(1+h/100))*(1+dmg/100.0);
    }
