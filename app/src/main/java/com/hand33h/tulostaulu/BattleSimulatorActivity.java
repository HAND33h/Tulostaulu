...
public class BattleSimulatorActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(16),dp(18),dp(16),dp(40));root.setBackgroundColor(Color.rgb(7,23,39));sc.addView(root);
        root.addView(title("⚔️ BATTLE SIMULATOR"));
        root.addView(info("Chief Gear käyttää nyt koko julkaistua micro-stage-taulukkoa aina Red T6 ★★★ asti: tarkka stat %, Deployment Capacity ja power jokaiselle valittavalle vaiheelle. Hero Expedition/Exclusive Weapon, Hero Gear, 18 Charms, Pets ja Experts säilyvät mukana. 3/6-piece Chief Gear set bonus tunnistetaan, mutta numeerista set-%:a ei arvata ilman varmennettua taulukkoa."));
    ...
    private void buildSide(boolean attacker){
        ...
        root.addView(sub("🛡️ Chief Gear — full progression"));
        Spinner[] gs=attacker?aGear:dGear;
        for(int i=0;i<6;i++){root.addView(tiny(GEAR_SLOTS[i]));gs[i]=spinner(ChiefGearData.NAMES);root.addView(gs[i]);}

        root.addView(sub("💎 Chief Charms — all 18 slots"));
        Spinner[] cs=attacker?aCharm:dCharm;
        for(int piece=0;piece<6;piece++){root.addView(tiny(GEAR_SLOTS[piece]+" charms"));for(int j=0;j<3;j++){int idx=piece*3+j;cs[idx]=spinner(CHARM_LEVELS);root.addView(cs[idx]);}}
    ...
    private void simulate(){
        try{
            ...
            String aChief=applyGearAndCharms(A,aGear,aCharm),dChief=applyGearAndCharms(D,dGear,dCharm);
            ...
            double aLoss=Math.min(100,Math.max(5,72*(ds/(as+ds)))),dLoss=Math.min(100,Math.max(5,72*(as/(as+ds))));
            long ar=Math.max(0,Math.round(A.count*(1-aLoss/100))),dr=Math.max(0,Math.round(D.count*(1-dLoss/100)));
            result.setText(String.format(Locale.US,
                    "🏆 Predicted winner: %s\n\nWin estimate\nAttacker %.1f %% | Defender %.1f %%\n\nEffective weighted stats\nA: ATK %.2f DEF %.2f HP %.2f LETH %.2f DMG +%.2f%%\nD: ATK %.2f DEF %.2f HP %.2f LETH %.2f DMG +%.2f%%\n\nEffective squad size\nA: %,.0f | D: %,.0f\n\nCombat score\nA: %,.0f | D: %,.0f\n\nEstimated losses\nA: %.1f %% (~%,d left)\nD: %.1f %% (~%,d left)\n\nCHIEF GEAR / CHARMS — ATTACKER\n%s\n\nCHIEF GEAR / CHARMS — DEFENDER\n%s\n\nPET\nA: %s\nD: %s\n\nAUTO HERO DATA — ATTACKER\n%s\n\nAUTO HERO DATA — DEFENDER\n%s",
                    p>=.5?"ATTACKER":"DEFENDER",p*100,(1-p)*100,A.s.atk,A.s.def,A.s.hp,A.s.leth,A.s.damageDealt,D.s.atk,D.s.def,D.s.hp,D.s.leth,D.s.damageDealt,A.count,D.count,as,ds,aLoss,ar,dLoss,dr,aChief,dChief,apr.note,dpr.note,aHeroNotes,dHeroNotes));
        }catch(Exception e){result.setText("⚠️ Tarkista syötetyt arvot. "+e.getClass().getSimpleName()+": "+e.getMessage());}
    }
    ...
    private String applyHeroes(Side own,Side enemy,Spinner[] heroes,Spinner[] skills,Spinner[] exclusive,boolean attackerSide){
        ...
        for(int i=0;i<3;i++){
            ...
            notes.append("• ").append(HeroBattleData.nameOf(selected)).append(" skill Lv").append(lv).append(": ");if(e.hasVerifiedSkillData)notes.append(e.note);else notes.append("skill semantics not yet auto-scored");notes.append(" | ").append(HeroBattleData.exclusiveSummary(selected,ewLv)).append('\n');
        }return notes.length()==0?"No heroes selected":notes.toString().trim();
    }

    private String applyGearAndCharms(Side x,Spinner[] gear,Spinner[] charm){
        int[] pos=new int[6];for(int i=0;i<6;i++)pos[i]=gear[i].getSelectedItemPosition();
        double infGear=(ChiefGearData.stat(pos[2])+ChiefGearData.stat(pos[3]))/2.0;
        double lanGear=(ChiefGearData.stat(pos[0])+ChiefGearData.stat(pos[1]))/2.0;
        double marGear=(ChiefGearData.stat(pos[4])+ChiefGearData.stat(pos[5]))/2.0;
        double weighted=(x.inf*infGear+x.lan*lanGear+x.mar*marGear)/100.0;x.s.atk+=weighted;x.s.def+=weighted;
        int cap=0;long power=0;for(int p:pos){cap+=ChiefGearData.deployment(p);power+=ChiefGearData.power(p);}x.count+=cap;
        double infC=0,lanC=0,marC=0;for(int i=0;i<18;i++){double b=CHARM_BONUS[charm[i].getSelectedItemPosition()];if(i<6)lanC+=b;else if(i<12)infC+=b;else marC+=b;}infC/=6.0;lanC/=6.0;marC/=6.0;double c=(x.inf*infC+x.lan*lanC+x.mar*marC)/100.0;x.s.hp+=c;x.s.leth+=c;
        return String.format(Locale.US,"Chief Gear weighted ATK/DEF +%.2f%% | deployment +%,d | gear power %,d\nInf %.2f%% / Lancer %.2f%% / Marksman %.2f%% per-piece pair avg\nCharm weighted Health/Lethality +%.2f%%\n%s",weighted,cap,power,infGear,lanGear,marGear,c,ChiefGearData.setStatus(pos));
    }
    ...
    private void applyExpert(Side own,Side enemy,String name,int affinity,boolean skills){
        ...
        else if("Gareth".equals(name)){own.s.leth+=30*k;own.s.hp+=30*k;if(skills){own.s.def+=50;own.s.hp+=50;enemy.s.leth-=5;}}
    }
    ...
    private PetResult applyPet(Stats own,Stats enemy,String pet,int level){
        ...
        if("Mammoth".equals(pet)){own.def+=p[i];return new PetResult(0,"Defense +"+p[i]+"% (late intermediate stages not claimed exact)");}
        if("Frost Gorilla".equals(pet)){own.hp+=p[i];return new PetResult(0,"Health +"+p[i]+"%");}
        if("Frostscale Chameleon".equals(pet)){enemy.def-=p[i];return new PetResult(0,"Enemy Defense -"+p[i]+"% (late intermediate stages not claimed exact)");}
        if("Snow Leopard".equals(pet)){double red=(i>=9?5:new double[]{1.5,2,2.5,3,3.5,4,4,4,4,5}[i]);enemy.leth-=red;return new PetResult(0,"Enemy Lethality -"+red+"%; late intermediate stages not claimed exact");}
        if("Titan Roc".equals(pet)){double[] known={1.5,2,2.5,3,3.5,4,5};int k=Math.min(6,(int)Math.floor(i*6.0/9.0));double red=known[k];enemy.hp-=red;return new PetResult(0,"Enemy Health -"+red+"%; mapped to 7 published stages");}
