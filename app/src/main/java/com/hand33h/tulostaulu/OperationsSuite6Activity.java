...
public class OperationsSuite6Activity extends Activity {
    ...
    private EditText total,inf,lan,mark; private TextView formationResult;
    ...
    @Override protected void onCreate(Bundle b){
        ...
        hero("👑 WOS OPERATIONS SUITE 6.0",tr("Inventory → best next upgrade → battle reports → formations → alliance ops → event planning","Inventory → best next upgrade → battle reports → formations → alliance ops → event planning"));
        ...
        Button saveVault=button(tr("TALLENNA INVENTORY VAULT","SAVE INVENTORY VAULT"));saveVault.setOnClickListener(v->{SharedPreferences.Editor e=p.edit();put(e,"vault_fc",fc);put(e,"vault_rfc",rfc);put(e,"vault_alloy",alloy);put(e,"vault_polish",polish);put(e,"vault_plans",plans);put(e,"vault_amber",amber);put(e,"vault_guides",guides);put(e,"vault_designs",designs);put(e,"vault_secrets",secrets);put(e,"vault_petfood",petFood);put(e,"vault_manuals",manuals);put(e,"vault_potions",potions);put(e,"vault_serums",serums);put(e,"vault_heroxp",heroXp);put(e,"vault_essence",essence);put(e,"vault_mythril",mythril);put(e,"vault_speedups",speedups);e.apply();toast(tr("Inventory tallennettu","Inventory saved"));});root.addView(saveVault,mp(0,6,0,14));

        section(tr("BEST NEXT UPGRADE","BEST NEXT UPGRADE"));
        root.addView(info(tr("Avaa Master Planner ja vertaa päivityksiä nykyiseen Inventory Vaultiin. Sovellus ei keksi power/material-suhteita, jos lähdedata puuttuu.","Open Master Planner and compare upgrades against the current Inventory Vault. The app does not invent power/material ratios when source data is missing.")));
        ...
        root.addView(info(tr("Lue useita saman taistelun raporttikuvia yhdellä kertaa, kerää troop/stat-havaintoja ja tallenna viimeisin analyysi.","Read multiple screenshots from the same battle, collect troop/stat observations and keep the latest analysis.")));
        addLaunch("⚔️",tr("Battle Report Analyzer 6.0","Battle Report Analyzer 6.0"),BattleReportAnalyzerActivity.class);
        ...
        root.addView(info(tr("Seuraavia officer-tietoja voi säilyttää paikallisina muistiinpanoina. Tämä ei lähetä niitä ulkopuoliselle palvelulle.","The following officer data can be kept as local notes. Nothing is sent to an external service.")));
        ...
        root.addView(info(tr("Tietokerros erottaa Verified WOS Data / OCR-FID Observed Data / Community Advice. Datasetit pitää versioida lähteellä, verified_at-ajalla ja checksumilla ennen automaattista käyttöönottoa. Tämän version local schema on 6.0 ja verified baseline 2026-09-08.","The data layer separates Verified WOS Data / OCR-FID Observed Data / Community Advice. Datasets should be versioned with source, verified_at and checksum before automatic activation. This version uses local schema 6.0 and verified baseline 2026-09-08.")));
