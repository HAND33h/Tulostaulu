...
public class Wos73Activity extends Activity {
    ...
    private TextView koiResult,bannerResult,champResult,historyResult;
    ...
    @Override protected void onCreate(Bundle b){super.onCreate(b);p=getSharedPreferences(PREFS,MODE_PRIVATE);en="en".equals(p.getString("lang","fi"));
        ...
        title("👑 WOS COMMAND CENTER 7.3",tr("KOI • Banners • Ministers • Roster • Castle • Championship • History","KOI • Banners • Ministers • Roster • Castle • Championship • History"));

        section("7-DAY KOI PLANNER");
        root.addView(info(tr("Syötä tämän eventin live-pistearvot ja oma tavoite. Päiväkohtaisia pisteitä ei kovakoodata, jotta planneri toimii season-muutoksissa.","Enter the live values from the current event and your target. Day values are not hardcoded so the planner remains valid across seasons.")));
        ...
        root.addView(info(tr("Alliance banner -resurssibudjetti. Syötä yhden bannerin nykyinen kustannus suoraan pelistä ja määrä.","Alliance banner resource budget. Enter the current per-banner costs directly from the game and quantity.")));
        ...
        historyNote=note(tr("Lisää oma merkintä historiaan","Add a note to history"),"");root.addView(historyNote);Button hs=button(tr("TALLENNA HISTORIAAN","SAVE TO HISTORY"));historyResult=info(loadHistory());hs.setOnClickListener(v->{saveHistory("Manual",historyNote.getText().toString());historyResult.setText(loadHistory());historyNote.setText("");});root.addView(hs);root.addView(historyResult);
        Button hr=button(tr("PÄIVITÄ HISTORIA","REFRESH HISTORY"));hr.setOnClickListener(v->historyResult.setText(loadHistory()));root.addView(hr);

        setContentView(sc);
    }

    private void calcKoi(){long goal=n(koiGoal),have=n(koiHave),days=Math.max(1,n(koiDays));long gap=Math.max(0,goal-have);long daily=(long)Math.ceil(gap/(double)days);String out=tr("Pistevaje: ","Point gap: ")+fmt(gap)+"\n"+tr("Minimi / jäljellä oleva päivä: ","Minimum / remaining day: ")+fmt(daily)+"\n\n"+tr("Käytä KOI-notes-kenttää nykyisen päivän live-pistearvoihin.","Use KOI notes for the current day's live point values.");koiResult.setText(out);p.edit().putString("koi_goal",koiGoal.getText().toString()).putString("koi_have",koiHave.getText().toString()).putString("koi_days",koiDays.getText().toString()).putString("koi_notes",koiNotes.getText().toString()).apply();saveHistory("KOI",out);}
