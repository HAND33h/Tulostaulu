...
public class ExpertSettingsActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){super.onCreate(b);p=getSharedPreferences(PREFS,MODE_PRIVATE);ScrollView sc=new ScrollView(this);root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(28,30,28,50);root.setBackgroundColor(Color.rgb(7,23,39));sc.addView(root);
        TextView h=t("🧠 EXPERT BONUS CENTER",24,true,Color.WHITE);root.addView(h);root.addView(t("Syötä vain omassa pelissä näkyvät Expert-bonukset. Yhteinen profiili välittää varmennetut arvot niitä tukeviin laskureihin. Sovellus ei arvaa puuttuvia Expert-arvoja.",13,false,Color.rgb(205,226,238)));
    ...
    private void save(){
        ...
        e.apply();
        p.edit().putString("expert_effective_combat_summary",ExpertBonusStore.summary(this)).apply();
        Toast.makeText(this,"Expert-bonukset päivitetty",Toast.LENGTH_SHORT).show();
    }
